...
namespace QuantumTek.QuantumDialogue.Editor
{
    ...
    public class QD_ChoiceNode : QD_Node
    {
        ...
        public override void DrawWindow(int id)
        {
            ...
            bool isAction = Data.isAction;
            isAction = EditorGUI.Toggle(new Rect(5, 40, 200, 20), "isAction", isAction);
            List<string> fChoices = new List<string>();
            foreach (var choice in Data.Choices)
                fChoices.Add(choice);

            int count = Data.Choices.Count;
            if (GUI.Button(new Rect(140, 70 + count * 30, 20, 20), "+", QD_DialogueEditor.skin.button) && Data.Choices.Count < 6)
            {
            ...
            {
                EditorGUI.LabelField(new Rect(5, 70 + i * 30, 65, 20), "Choice " + (i + 1), QD_DialogueEditor.skin.label);
                fChoices[i] = EditorGUI.TextField(new Rect(75, 70 + i * 30, 195, 20), fChoices[i], QD_DialogueEditor.skin.textField);

                if (i == count - 1)
                {
                    if (GUI.Button(new Rect(275, 72.5f + i * 30, 20, 20), "-", QD_DialogueEditor.skin.button))
                    {
                        EditorUtility.SetDirty(QD_DialogueEditor.db);
                        EditorUtility.SetDirty(QD_DialogueEditor.db.DataDB);
                        fChoices.RemoveAt(i);
