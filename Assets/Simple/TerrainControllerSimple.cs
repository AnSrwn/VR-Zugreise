...
public class TerrainControllerSimple : MonoBehaviour {
    ...
    private void Update() {
        ...
        Vector2 playerTile = TileFromPosition(playerTransform.position - terrainRoot.transform.position);
        ...
        foreach (Transform t in gameTransforms)
            centerTiles.Add(TileFromPosition(t.position - terrainRoot.transform.position));
    ...
    private GameObject CreateTile(int xIndex, int yIndex) {
        GameObject terrain = Instantiate(
            terrainTilePrefab,
            new Vector3(terrainSize.x * xIndex, terrainSize.y-2, terrainSize.z * yIndex) + terrainRoot.transform.position,
            Quaternion.identity
        );
        terrain.name = TrimEnd(terrain.name, "(Clone)") + " [" + xIndex + " , " + yIndex + "]";
        terrain.transform.parent = terrainRoot.transform;
